...
struct TaskRequest {
  ...
  TaskRequest(int root_rank, int layer_id, int blob_id, int part_id_) :
  ...
  void ParseInfo(int64_t mpi_rank, int64_t mpi_tag) {
    ...
    mpi_tag -= layer_id * 1e6;
    blob_id_ = mpi_tag / 1e4;
    mpi_tag -= blob_id * 1e4;
    part_id_ = mpi_tag;
  }
  int GetTag() {
    return layer_id_ * 1e6 + blob_id_ * 1e4 + part_id;
  }
};
    ...
    for (int i = 0; i < caffe::internode::nGroup; i++) {
      ...
      for (int j = 0; j < solver_->net()->layers().size; j++)
        for (int k = 0; k < solver->net()->layers()[j].size; k++) {
          ...
          buf_ptr_.insert(make_pair(make_pair(root_rank, make_pair(j, k) ), NULL) );
          std::free(buf_ptr_[make_pair(make_pair(root_rank, make_pair(j, k) ) ) ] );
        }
    }
  };
...
private:
  ...
  std::queue<TaskRequest> update_tasks_;
  std::queue<TaskRequest> send_tasks_;
  ...
  // for computation
  boost::shared_ptr<MultiSolver<Dtype> > solver_;
